...
def load_data(path):
    ...
    for file in os.listdir(path):
        ...
        try:
            image = np.array(cv2.resize(cv2.imread(file_path), (IMG_WIDTH, IMG_HEIGHT), interpolation=cv2.INTER_AREA))
            images.append(image)
            if n % 10000 == 0:
                print(n)
            n += 1
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(12*NUM_CATEGORIES, activation='relu'),
